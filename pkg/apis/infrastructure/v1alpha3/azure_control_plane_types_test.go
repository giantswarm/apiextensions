...
func Test_GenerateAzureControlPlaneYAML(t *testing.T) {
	testCases := []struct {
		category string
		name     string
		...
		{
			category: "cr",
			name:     fmt.Sprintf("%s_%s_azurecontrolplane.yaml", group, version),
			resource: newAzureControlPlaneExampleCR(),
		},
	}
...
func newAzureControlPlaneExampleCR() *AzureControlPlane {
	cr := NewAzureControlPlaneCR()

	cr.Name = "ier2s"
	cr.Spec = AzureControlPlaneSpec{
